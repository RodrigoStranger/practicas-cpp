...
void verificarSalud(double promedioAnteriorPeso, const vector<double> &pesos) {
    ...
    if(resultado > 0) {
        subieronPeso++;
        cout << "Persona " << personas << " usted subio " << abs(resultado) <<  "kg de peso" << endl;
    } else if (resultado == 0 ) {
        cout << "Usted mantiene el mismo peso" << endl;
    } else {
        bajaronPeso++;
        cout << "Persona " << personas << " usted bajo " << abs(resultado) <<  "kg de peso" << endl;
    }
}
